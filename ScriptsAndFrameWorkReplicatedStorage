...
function NumberFormatter.Format(n)
	...
	-- 2. Calculate the thousand-step (exponent)
	...
	if exp < #standardSuffixes then
		...
		-- Calculate how far past Decillion we are
		local romanIndex = (exp - #standardSuffixes) + 1
		local numeral = romanNumerals[romanIndex] or "∞"

		-- Returns "1.23 ΩI", "1.23 ΩII", etc.
		-- (No space between Omega and Numeral for that tight 'Rank' look)
